...
from utils.database import (
    ...
    InsertMemberStatusToGroupBlockDB,
)
...
async def GroupWarnMember(app: GraiaMiraiApplication, event: GroupMessage):
    ...
    for i in target:
        ...
        if not InsertMemberStatusToGroupBlockDB(
            app, event.sender.group, i, "Warn", times
        ):
            return
        if times == 1:
            ...
            block = GetMemberStatusFromGrouBlockDB(
            ...
            block += 1
            if not InsertMemberStatusToGroupBlockDB(
                app, event.sender.group, i, "Blocked", block
            ):
                return
...
async def GroupCancelWarnMember(app: GraiaMiraiApplication, event: GroupMessage):
    ...
    if not checkMemberPermission(
        app, event.sender, [MemberPerm.Administrator, MemberPerm.Owner], qutoed
    ):
        return
    target = getTargetFromAt(app, event.sender.group, event.messageChain)
    succ = []
...
async def GroupBlockMember(app: GraiaMiraiApplication, event: GroupMessage):
    ...
    if not checkBotPermission(
        ...
        [
            ...
            *(
                ...
                if event.sender.permission == MemberPerm.Member
                else []
            ),
        ],
        quoted,
    ...
    for i in target:
        ...
        if times == None:
            return
        if not InsertMemberStatusToGroupBlockDB(
            app, event.sender.group, i, "Blocked", times + 1
        ):
            return
    ...
    await app.sendGroupMessage(
        event.sender.group,
        MessageChain.create([Plain(f"操作完成.被拉黑的对象为:\n{' '.join(succ)}")]),
    )
...
async def GroupUnBlockMember(
    ...
    for i in target:
        ...
        if times == None:
            return
        if not InsertMemberStatusToGroupBlockDB(
            app, event.sender.group, i, "Blocked", 0
        ):
            return
        succ.append(i.id)
    await app.sendGroupMessage(
