...
async def BotJoinGroup(app: GraiaMiraiApplication, event: BotJoinGroupEvent):
    await app.sendGroupMessage(
        event.group, MessageChain.create([Plain("引入了新的机器人时要小心命令冲突哦!")])
    )
...
async def RequsetBotMaster(
    ...
    try:
        ...
        async def wait(
            ...
            if (
                not quote
                or wait_event.sender.id != target
        ...
        raise ReturnValue(
            [await Interrupt.interruptcontrol.wait(wait), target, botMessage.messageId]
        )
    except asyncio.CancelledError:
        await asyncio.sleep(randint(1, 10))
        await app.sendFriendMessage(
            target, MessageChain.create([Plain("已由其他人处理")]), quote=botMessage.messageId
        )


async def BotInviteJoinGroup(
    app: GraiaMiraiApplication, event: BotInvitedJoinGroupRequestEvent
):
