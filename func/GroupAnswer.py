from graia.application.entry import GraiaMiraiApplication,GroupMessage,Source,MessageChain,Member,MemberPerm
...
from utils.database import InsertToGroupDB,DeleteFromGroupDB,GetAllFromGroupDB,GetMemberStatusFromGrouBlockDB,CheckIfInGroupDB
from utils.messageTrigger import regexPlain,strictPlainCommand


async def GroupAddAnswer(app:GraiaMiraiApplication,event:GroupMessage,regexResult=regexPlain(r'^#添加进群答案[\s]*([^\s]*)$')):
...
async def GroupDeleteAnswer(app:GraiaMiraiApplication,event:GroupMessage,regexResult=regexPlain(r'^#删除进群答案[\s]*([^\s]*)$')):
    quoted=event.messageChain.get(Source)[0]
    if not await checkMemberPermission(app,event.sender,[MemberPerm.Administrator,MemberPerm.Owner],quoted):
        return 
    answer=regexResult.groups()[0]
...
def AddGroupAnswerListener(bcc:Broadcast):
    ...
    bcc.receiver("GroupMessage",headless_decoraters=[strictPlainCommand("#可用进群答案")])(GroupAllowAnswer)
