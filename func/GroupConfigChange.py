...
async def GroupPermissionChange(
    app: GraiaMiraiApplication, event: BotGroupPermissionChangeEvent
):
    if event.origin == event.current:
        await app.sendGroupMessage(
            event.group, MessageChain.create([Plain("为啥会发出这句?这是发生了啥?我不知道,不要问我.")])
        )
    elif event.current == MemberPerm.Member:
        await app.sendGroupMessage(
            event.group, MessageChain.create([Plain("哦豁,管理权限没了.")])
        )
    elif event.current == MemberPerm.Administrator:
        await app.sendGroupMessage(
            event.group, MessageChain.create([Plain("权限升级,功能升级(雾)")])
        )
    elif event.current == MemberPerm.Owner:
        await app.sendGroupMessage(
            event.group, MessageChain.create([Plain("我咋成群主了?发生了啥?")])
        )
    else:
        await app.sendGroupMessage(
            event.group, MessageChain.create([Plain("为啥会发出这句?这是发生了啥?我不知道,不要问我.")])
...
async def GroupNameChange(app: GraiaMiraiApplication, event: GroupNameChangeEvent):
    if event.isByBot:
        return
    await app.sendGroupMessage(
        event.group, MessageChain.create([Plain("这是发生了啥?为啥要改群名?")])
    )


async def GroupEntranceAnnouncementChange(
    app: GraiaMiraiApplication, event: GroupEntranceAnnouncementChangeEvent
):
    await app.sendGroupMessage(
        event.group,
        MessageChain.create(
            [
                ...
                (
                    Plain("\n此处应有@全体")
                    if event.group.accountPerm
                    in [MemberPerm.Owner, MemberPerm.Administrator]
                    else Plain("\n既然不具备权限就不@全体了")
                ),
            ]
        ),
...
async def GroupAnonymousChat(
    app: GraiaMiraiApplication, event: GroupAllowAnonymousChatEvent
):
    await GroupSettingChanged(
        app,
        event,
...
async def GroupConfessTalk(
    app: GraiaMiraiApplication, event: GroupAllowConfessTalkEvent
):
    await GroupSettingChanged(
        app,
        event,
