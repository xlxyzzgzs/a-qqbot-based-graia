...
async def muteAll(
    app: GraiaMiraiApplication, group: Group, quote: Optional[Union[Source, int]] = None
) -> bool:
    if not await checkBotPermission(
        app, group, [MemberPerm.Owner, MemberPerm.Administrator], quote
    ):
        return False
    try:
