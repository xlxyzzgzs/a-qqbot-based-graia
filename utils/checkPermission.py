...
async def checkBotPermission(
    app: GraiaMiraiApplication,
    group: Group,
    needPermission: List[MemberPerm],
...
async def checkMemberPermission(
    app: GraiaMiraiApplication,
    member: Member,
    needPermission: List[MemberPerm],
    quote: Optional[Union[Source, int]] = None,
    ...
    if (
        member.permission in needPermission
        or member in BotAdmin
        or member in BotMaster
        or GetPermissionFromDB(app, member) in needPermission
    ):
        return True
    await app.sendGroupMessage(
        member.group,
